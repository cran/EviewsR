...
* Don't do anything if the name of EViews executable is one of the following: `EViews12_x64`, `EViews12_x86`, `EViews11_x64`, `EViews11_x86`, `EViews10`. The package will find the executable automatically.
...
The `eviews` chunk automatically returns the outputs of each equation object as a dataframe, accessible via `eviews$equationName`. For example, The $R^2$ of the `ols` equation object is `r eviews$ols$r2[1]`, which can be accessed using `` `r knitr::inline_expr('eviews$ols$r2[1]')` ``.
...
```{r eviewsGraph,fig.cap="EviewsR example figure",out.width='80%',out.height='80%',fig.path=""}
